...
MSBF (**M**essage **S**tudio **B**inary **F**lowchart) is a file format that stores flowcharts used for event flows.
...
| 0x2    | u16         | [ATR1](/documentation/file-formats/lms/msbt) file index into folder or archive |
...
The next node index when marked as `-1` is the end of a flowchart unless it is a branch node. The next node for a jump node must refer to the ID of the entry node for another flowchart.
...
This block contains the flowchart [labels](../overview/#labels). The index of a flowchart is the location of its [Entry node](#entry-node).
